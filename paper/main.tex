...
\begin{tabular}{llll}
\toprule
LLVM IR (pseudocode) & encoding & LLVM IR (pseudocode) & encoding\\
\midrule
\texttt{z = add x, y} & $\den{z} = \den{x} + \den{y}$ \ (sub, mul alike) &
\texttt{r,o = sadd.ovf(x,y)} & $\den{r} = \den{x}+\den{y}$;\ $\den{o} = \neg\,\mathit{noOvf}_s(\den{x},\den{y})$\\
\texttt{z = udiv x, y} & $\den{z} = \den{x} \div_u \den{y}$ \ (sdiv, u/srem alike) &
\texttt{z = umax(x, y)} & $\mathrm{ite}(\den{x} >_u \den{y}, \den{x}, \den{y})$ \ (min/abs/fshl alike)\\
\texttt{z = shl x, y} & $\den{z} = \den{x} \ll \den{y}$ \ (lshr, ashr alike) &
\texttt{z = add nsw x, y} & add rule, plus assert $\mathit{noOvf}_s(\den{x},\den{y})$\\
\texttt{z = and x, y} & bitwise on BV; Boolean on \texttt{i1} &
...
\texttt{z = zext x} & zero-extend \ (sext, trunc alike) &
\texttt{br c, S, T} (edge to $S$) & $\mathit{edge} = \den{c}$ \ (to $T$: $\neg\den{c}$; switch: equalities)\\
\texttt{z = select c, x, y} & $\den{z} = \mathrm{ite}(\den{c}, \den{x}, \den{y})$ &
header phi, load, call, gep & fresh variable \ (boundary; \S\ref{sec:system-facts})\\
\bottomrule
\end{tabular}
\end{table*}
