...
\item \textbf{Heap-invariant super-analysis} (\S\ref{sec:frame}):
We identify a crucial analysis problem 
...
\item \textbf{Robust Performance Improvement} (\S\ref{sec:method} \& \S\ref{sec:results}):
  ...
  Runtime experiments with 30-repetition medians on two target architectures (x86 and M-series) show \emph{when} elimination pays: removing hot-loop checks has a cascading effect, unlocking further transformations (unrolling, vectorization, etc) that enables up to $4.16\times$ speedups for some widely used kernels. \tool{} proofs are generally cheap: \pct{96} of
...
Two things are needed. The first is a new fact source: when the
...
cannot answer. The second is a stricter encoding of the values the
optimizer introduces around a vectorized loop. Left unconstrained,
each of them is a free variable, and a single free variable in the
trap condition makes the query satisfiable no matter how strong the
other facts are; encoding each one by its definition removes that
escape. Both are changes to the encoder alone. The solver still only
