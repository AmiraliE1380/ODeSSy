...
for t in 16..<64 {        // schedule extension
  // s0, s1: rotate-xors of w[t-15], w[t-2]
  w[t] = w[t-16] &+ s0 &+ w[t-7] &+ s1
} // &+ = wrapping add: the author already
  // opted out of overflow checks; the four
  // BOUNDS checks per iteration remain
\end{verbatim}
Each subscript lowers to a compare-and-branch; for \texttt{w[t-16]}:
\begin{verbatim}
%n   = load i64, ptr %w.count      ; = 64
%oob = icmp uge i64 %idx, %n       ; idx = t-16
br i1 %oob, label %trap, label %ld ; ubsantrap
\end{verbatim}
The facts that refute the trap edge live in different analyses:
\begin{itemize}\setlength\itemsep{1pt}
\item loop bounds (scalar evolution): $t \in [16, 64)$, hence
  $\mathit{idx} = t - 16 \in [0, 48)$ --- exact in bit-vectors;
\item allocation (return-value range, refined by LazyValueInfo):
  $\texttt{w.count} = 64$;
\item the guard chain dominating the loop entry.
\end{itemize}
The trap requires $\mathit{idx} \geq \texttt{w.count}$; with
$\mathit{idx} < 48$ and $\texttt{w.count} = 64$ the conjunction is
absurd --- but no single fact suffices (drop any one and the query is
satisfiable), and no fixed pass computes the conjunction.
\caption{The simple case: the message-schedule extension of a Swift
SHA-256 kernel. Every one of these bounds checks survives
\texttt{swiftc -O} \emph{and} a second full LLVM O3; \tool{} conjoins
the facts, deletes the checks, and the unlocked unrolling is worth
\pct{8.8} on x86 (\S\ref{sec:results-perf}).}
\Description{A Swift listing of the SHA-256 message schedule loop
indexing array w at t minus 16, t minus 7, t minus 15, and t minus 2;
below it, three lines of LLVM IR showing the load of the array count,
the unsigned compare of the index against it, and the conditional
branch to a trap block; then a bulleted list of the three facts whose
conjunction refutes the trap.}
...
for j in 1:n, l in 1:k
    b = B[l, j]
    for i in 1:m
        C[i, j] += A[i, l] * b   # 16 trap edges
    end
end
\end{verbatim}
after \texttt{-O3}, schematically:
\begin{verbatim}
guard:  %n1 = load i64, ptr %A.size     ; == m
  ...
loop:   ; vectorized body: stores to C's data
check:  %n2 = load i64, ptr %A.size     ; fresh
        %ok = icmp ult i64 %i.frozen, %n2
        br i1 %ok, %body, %trap
\end{verbatim}
...
\Description{A Julia source listing of a three-loop GEMM kernel with a
dimension guard, followed by a schematic LLVM IR listing showing two
separate loads of the same size field, one constrained by the guard
and one consumed by the bounds check inside the loop.}
...
\subsection{Discharging heap invariants: the frame tier}
...
\frac{\mathrm{clobber}\bigl(L_2,\ \mathrm{loc}(L_1)\bigr) \ \preceq\
      \mathrm{def}(L_1)}
     {\ |\mathrm{FRAME}{:}k|:\ \ \llbracket L_1 \rrbracket =
...
guard-side load carries Julia's scoped no-alias metadata, under which
stock \textsc{ScopedNoAliasAA} proves every loop-carried store
...
$\llbracket \mathit{freeze}(x) \rrbracket = \llbracket x \rrbracket$
...
\[
\llbracket \phi \rrbracket - \mathit{start} \ \le_u\
s \cdot \mathrm{BTC}
\qquad\text{instead of}\qquad
\llbracket \phi \rrbracket \ \le_u\ \mathit{start} + s \cdot
\mathrm{BTC},
\]
...
Our evaluation is organized around four questions, one per contribution
...
\item \textbf{RQ3 --- Can the heap-invariant frontier be crossed?}
  The GEMM acceptance test of \S\ref{sec:frame} under the frame tier of
  \S\ref{sec:frame-machinery}: proof yield, end-to-end speedup through
  proof-licensed annotation, and ride-along gains
  (\S\ref{sec:frame-results}).
\end{itemize}
%% =====================================================================
...
\subsection{RQ3: Crossing the frontier --- 0 to 16-of-16}
\label{sec:frame-results}
On the GEMM acceptance test of \S\ref{sec:frame}, the frame tier of
\S\ref{sec:frame-machinery} proves \textbf{all 16} trap edges, where
