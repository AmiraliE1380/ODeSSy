...
The checks are not exotic --- they guard
...
removed checks in Swift SHA-256 (5 to 7 proofs), Julia SHA-256 (4 to
10), a filter kernel from the DSP.jl library (0 to 6), and the
CryptoSwift library (183 to 215) --- not a matrix kernel among them
(\S\ref{sec:frame-results}). The fact that
would discharge the check is \emph{nonlocal}: the guard and the check
are linked only by the theorem that their two loads agree, and that
theorem's premise --- no intervening write touches the location ---
quantifies over every store in the loop nest. A dataflow analysis
propagates facts \emph{about values}; this premise is a fact
\emph{about memory}. The pipeline's owners of memory facts (GVN,
alias analysis) each see only a fragment, and by the time the
vectorizer has cloned the check-side loads and stripped their
scoped-aliasing metadata, LLVM can no longer justify the unification
...
Two requirements follow. First, a sound, refusing importer for
load-equality facts, whose premise is discharged by the compiler's own
memory analyses --- the frame rule of separation
logic~\cite{reynolds2002separation} recast as a fact source. Second,
an encoding in which the stabilized values the vectorizer manufactures
(\texttt{freeze}, resume indices, remainder-loop starts) are pinned by
\emph{definitional axioms} instead of being left free, since a free
variable in the trap condition makes every query satisfiable no matter
how strong the surrounding facts are. Both are additions to the
encoder, not to the transform: the solver still only answers
reachability, and the compiler still performs every rewrite, so the
super-analysis contract of \S\ref{sec:superanalysis} is preserved.
\S\ref{sec:frame-machinery} gives the rules.
%% =====================================================================
