...
\begin{tabular}{ll}
\toprule
LLVM IR (pseudocode) & encoding\\
\midrule
\texttt{z = add x, y} & $\den{z} = \den{x} + \den{y}$ \hfill (sub, mul alike)\\
\texttt{z = udiv x, y} & $\den{z} = \den{x} \div_u \den{y}$ \hfill (sdiv, u/srem alike)\\
\texttt{z = shl x, y} & $\den{z} = \den{x} \ll \den{y}$ \hfill (lshr, ashr alike)\\
\texttt{z = and x, y} & bitwise on BV; Boolean on \texttt{i1} \hfill (or, xor alike)\\
\texttt{b = icmp ult x, y} & $\den{b} = \den{x} <_u \den{y}$ \hfill (all ten predicates)\\
\texttt{z = zext x} & zero-extend \hfill (sext, trunc: extend/extract)\\
\texttt{z = select c, x, y} & $\den{z} = \mathrm{ite}(\den{c}, \den{x}, \den{y})$\\
\texttt{r,o = sadd.overflow(x,y)} & $\den{r} = \den{x}+\den{y}$;\ $\den{o} = \neg\,\mathit{noOvf}(\den{x},\den{y})$\\
\texttt{z = umax(x, y)} & $\mathrm{ite}(\den{x} >_u \den{y}, \den{x}, \den{y})$ \hfill (min/abs/fshl/bswap alike)\\
\texttt{z = add nsw x, y} & rule above, plus assert $\mathit{noOvf}_s(\den{x},\den{y})$\\
\texttt{z = freeze x} & $\den{z} = \den{x}$ \hfill (identity; \S\ref{sec:frame-machinery})\\
\texttt{z = phi [v$_k$, B$_k$]} & nested $\mathrm{ite}$ over $\mathit{Reach}(B_k) \wedge \mathit{edge}(B_k{\to}B)$\\
\texttt{br c, S, T} (edge to $S$) & $\mathit{edge} = \den{c}$ \hfill (to $T$: $\neg\den{c}$; switch: case equalities)\\
loop-header phi, load, call, gep & fresh variable \hfill (boundary; \S\ref{sec:system-facts}, \S\ref{sec:frame-machinery})\\
\bottomrule
\end{tabular}
\end{table}
