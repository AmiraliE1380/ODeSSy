...
for t in 16..<64 {   // schedule extension
  let s0 = rotr(w[t-15],7) ^ rotr(w[t-15],18)
           ^ (w[t-15] >> 3)
  let s1 = rotr(w[t-2],17) ^ rotr(w[t-2],19)
           ^ (w[t-2] >> 10)
  w[t] = w[t-16] &+ s0 &+ w[t-7] &+ s1
}
\end{verbatim}
\caption{The simple case: the message-schedule extension of a Swift
SHA-256 kernel. Every subscript is a native bounds check --- a compare
and a conditional branch to a trap block in the IR --- and every one of
them survives \texttt{swiftc -O} \emph{and} a second full LLVM O3.
Proving \texttt{w[t-16]} in range needs \texttt{t} $\in [16, 64)$ (a
scalar-evolution fact about the loop), \texttt{w.count} $= 64$ (a
return-value range from the allocation, refined by LazyValueInfo), and
the dominating guard chain --- facts owned by \emph{different} analyses
that no fixed pass intersects. \tool{} conjoins them and deletes the
checks; the unlocked unrolling is worth \pct{8.8} on x86
(\S\ref{sec:results-perf}).}
\Description{A nine-line Swift code listing of the SHA-256 message
schedule loop, indexing the array w at t minus 15, t minus 2, t minus
16, and t minus 7 inside a loop over t from 16 to 64.}
