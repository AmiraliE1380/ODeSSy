...
\end{lstlisting}
\end{minipage}
\caption{The final boss: Julia stdlib GEMM (left: the source, with the
check on \texttt{A[i,l]} spelled out in red; right: the same check
...
tagged n2, and the accumulate line tagged st. Right, a schematic LLVM
...
one second, annotated with an UNSAT maximum of 2238 milliseconds; no
red bar reaches that far.}
\label{fig:latency}
\end{figure}
\begin{table}
