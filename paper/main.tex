...
\begin{lstlisting}[style=odessy,
    morekeywords={load,store,add,icmp,uge,br,label,i64,i1,ptr,freeze,x,double},
    comment={[l]{;}},
    moredelim={[is][\color{trapred}\bfseries]{|}{|}}]
; [n1] before the loops -- pinned by the guard:
%n1 = load i64, ptr %A.size       ; A's row count, == m
; ... one iteration of the vectorized inner loop:
store <2 x double> %v, ptr %C.data ; [st] may it hit %A.size?
%n2 = load i64, ptr %A.size       ; [n2] SAME address, reloaded
%idx = add i64 %i, -1             ; zero-based index
|%oob = icmp uge i64 %idx, %n2    ; oob <=> !(1 <= i <= n2)|
|%oob.fr = freeze i1 %oob         ; vectorizer freezes the flag|
|br i1 %oob.fr, label %trap, label %body|
\end{lstlisting}
\end{minipage}
...
after lowering; \texttt{[n1]}, \texttt{[n2]}, \texttt{[st]} tie the
two sides together). Julia's one-based check $1 \le i \le n$ lowers to
a single unsigned compare on the zero-based index,
$\mathit{oob} \iff (i{-}1) \ge_u n_2$, since a negative index wraps to
a huge unsigned value. The guard pins the first load of
\texttt{A}'s size field ($n_1 = m$); the check consumes a
\emph{second} load of the same address ($n_2$), and between the two
sits the loop's store into \texttt{C}'s data. Whether $n_1 = n_2$ is
therefore an aliasing question --- can that store touch
\texttt{A}'s size field? --- and the vectorizer has cloned the reload
without the scoped-aliasing metadata that would answer it, and has
wrapped the check's flag in \texttt{freeze}. Sound encoding must make
$n_2$ a fresh symbol and, read naively, the frozen flag a fresh
Boolean, so every query is trivially satisfiable no matter how many
scalar facts are supplied. What is missing is a \emph{heap
invariant} --- no store between the two loads touches the size
field, hence $n_1 = n_2$ --- plus the axiom that \texttt{freeze} is
the identity on defined values. \S\ref{sec:frame-machinery} gives the
encoding rules; \S\ref{sec:frame-results} reports the result --- 16
of 16, and $3.6$--$4.2\times$ end-to-end.}
...
IR listing: the guard-side load of the size field tagged n1, a
vectorized store into C tagged st, the reload of the size field tagged
n2, the zero-based index computation, and in red the unsigned
compare, its freeze, and the conditional branch to the trap block.}
