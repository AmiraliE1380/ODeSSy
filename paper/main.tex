...
\begin{table}
\caption{The dose--response table: every runtime experiment, sorted by
speedup (medians, REPS$\geq$21 shuffled reps, oracle vs.\ the double-O3
control, byte-identical-output gate; final encoder, full fact tier).
``Elim'' counts proven trap edges; ``Unlock'' names the transformation
the proofs gate.}
\label{tab:speedups}
\footnotesize
...
\toprule
Benchmark & Elim & Platform & Speedup & Unlock & Ceiling & Recov.\\
\midrule
gemm (Julia)$^{b}$ & 16 & M-series & $4.16\times$ & vec+unroll & $4.19\times$ & 99.8\%\\
gemm (Julia)$^{b}$ & 16 & x86 & $3.59\times$ & vec+unroll & $3.97\times$ & 96.4\%\\
sha256 (Swift) & 7 & x86 & $+8.8$\% & unroll & 9.0\% & 98\%\\
sha1 (Swift) & 7 & M-series & $+6.4/{+}7.3$\% & unroll & --- & ---\\
sha256 (Swift) & 7 & M-series & $+6.9$\% & unroll & --- & ---\\
adler32 (Swift) & 1 & x86 & $+3.7$\% & DO16 group & 11.6\% & 32\%\\
zstd comp.\ (C) & 6 & x86 & $+2.3$\% & hot group & 1.2\%$^{a}$ & ---$^{a}$\\
utf8 (Swift) & 2 & x86 & $+0.0$\% & none (cold) & 7.5\% & 0\\
CryptoSwift & 183 & x86 & $\pm0.3$\% & none (cold) & 10.4\%$^{c}$ & $\approx$0\\
zlib comp.\ (C) & 145 & x86 & $+0.1$\% & none (cold) & 5.4\%$^{a}$ & $\approx$0\\
zlib decomp.\ (C) & 113 & x86 & $+0.3$\% & none (cold) & 1.1\%$^{a}$ & $\approx$0\\
md5 (Swift) & 1 & x86 & $-1.8$\% & none (cold) & 6.0\% & $<0$\\
adler32 (Swift) & 1 & M-series & $-1.4$\% & none & --- & $<0$\\
sha1 (Swift) & 2 & x86 & $-5.0$\% ($\times4$) & none & 4.7\% & $<0$\\
\bottomrule
\end{tabular}\\
...
attack); zstd's \pct{2.3} \emph{exceeds} its overhead ceiling ---
eliminations also unlock re-optimization --- so no recovery fraction
is computed. $^{b}$Via proof-licensed \texttt{@inbounds}
(Table~\ref{tab:inbounds}); ceiling is the expert-annotation arm.
$^{c}$The raw \texttt{-O}-vs-\texttt{-Ounchecked} gap is \pct{19.6},
of which 8.1 points are the compilation pipeline (recovered by the
harness baseline itself) and \pct{10.4} are unchecked semantics ---
the honest ceiling.\par
\end{table}
...
The table's lower half is equally load-bearing. Where eliminations do not
