...
unsigned specification (\S\ref{sec:results-static}) --- checks that
\emph{fire} by design cannot be dead. The bound is always relative to
the fact vocabulary, however, and this paper measures both sides of
it. Julia's stdlib GEMM is irreducible to every \emph{scalar}
conjunction --- its own compiler strands a measured $4\times$ behind
those checks --- until the heap-invariant facts of \S\ref{sec:frame}
make all sixteen provable; what survives even that vocabulary is
priced rather than hidden, benchmark by benchmark, in
