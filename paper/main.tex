...
\begin{lstlisting}[style=odessy,
    morekeywords={var,for,in,let},
    comment={[l]{//}},
    moredelim={[l][\color{trapred}\bfseries]{//!}}]
var w = [UInt32](repeating: 0, count: 64)
for t in 16..<64 {
  // s0, s1: rotate-xors of w[t-15], w[t-2]
  w[t] = w[t-16] + s0 + w[t-7] + s1
  //! trap if t-16 >= w.count   -- one bounds
  //! check per subscript, four per iteration
}
\end{lstlisting}
\end{minipage}\hfill
\begin{minipage}[t]{0.50\textwidth}
\begin{lstlisting}[style=odessy,
    morekeywords={load,add,icmp,uge,br,label,i64,i1,ptr},
    comment={[l]{;}},
    moredelim={[l][\color{trapred}\bfseries]{;!}}]
...
br i1 %oob, label %trap, label %ld
;! %trap: call @llvm.ubsantrap -- program aborts
\end{lstlisting}
\end{minipage}
\caption{The simple case: the message-schedule extension of a Swift
SHA-256 kernel (left) and the lowered form of one of its four
per-iteration bounds checks (right). Every one of these checks survives
...
$\texttt{w.count} = 64$;\newline
$\bullet$~the guard chain dominating the loop entry.\newline
...
conjoins them, deletes the checks, and the unlocked unrolling is worth
