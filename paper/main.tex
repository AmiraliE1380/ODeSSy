...
\tool{} generates code up to $4.16\times$ faster than LLVM -O3 for a GEMM implemented in Julia, speeds up a Swift SHA-256 kernel by $1.11\times$, and delivers a
...
\tool{} is able to recover up to $4.16\times$ of performance by removing redundant sanitizers and enabling a cascade of further optimizations.
...
\item \textbf{Heap-invariant super-analysis} (\S\ref{sec:frame}): a crucial analysis problem we identified: Array optimizations often require heap reasoning to ensure their safety. We formalize the problem and show that super-analysis can more effectively reason over heap and memory safety of certain transformations in hot loops.
...
\item \textbf{Robust Performance Improvement} (\S\ref{sec:method} \& \S\ref{sec:results}):
  our systematic evaluation of \tool{} on two target architectures (x86 and M-series) show removing hot-loop checks has a cascading effect, unlocking further transformations (unrolling, vectorization, etc) that enables up to $4.16\times$ speedups for widely used kernels and libraries. \tool{} proofs are generally cheap: \pct{96} of
