...
source-level pseudocode) and the same check after lowering (right,
schematic). Every one of these checks survives
\texttt{swiftc -O} \emph{and} a second full LLVM O3. The single
\emph{unsigned} comparison covers both bounds at once: a negative index
becomes a huge unsigned value and fails the same test. The facts that
...
$\texttt{w.count} = 64$;\newline
$\bullet$~the guard chain dominating the loop entry, which is what
makes the scalar-evolution bound applicable on this path.\newline
