...
\begin{lstlisting}[style=odessy]
Slow_Loop_with_Safety_checks
\end{lstlisting}
\end{minipage}\hfill$\Longrightarrow$\hfill
\begin{minipage}[c]{0.48\linewidth}
\begin{lstlisting}[style=odessy,morekeywords={if,else}]
...
\Description{On the left, a single line naming a slow loop with safety
checks. An arrow points right, to an if-else: if the synthesized guard
holds, the fast loop runs; otherwise the original slow loop with safety
checks runs.}
\label{fig:guard}
\end{figure}
