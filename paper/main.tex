...
after lowering). Every call to
\texttt{size} becomes a load of a field in the array header into a
fresh SSA register, so the guard and the check see two \emph{different}
values, $n_1$ and $n_2$, read from the \emph{same} address. Julia's
one-based check $1 \le i \le n$ is shown for its upper half as one
unsigned compare against the reloaded size,
$\mathit{oob} \iff i >_u n_2$ (Julia's actual lowering compares
$i{-}1 \ge_u n_2$, which covers both bounds at once since a
non-positive $i{-}1$ wraps to a huge unsigned value). The guard pins
$n_1 = m$; between the two loads sits the loop's store into
\texttt{C}'s data. Whether $n_1 = n_2$ is therefore an aliasing
question --- can that store touch \texttt{A}'s size field? --- and
the vectorizer has cloned the reload without the aliasing
metadata that would let alias analysis prove the two loads equal.
Sound encoding must therefore make $n_2$ a fresh symbol, and the trap
stays satisfiable no matter how many scalar facts are supplied. What
is missing is a \emph{heap invariant}:
no store between the two loads touches the size field, hence
$n_1 = n_2$. \S\ref{sec:frame-machinery} gives the encoding rules;
...
\end{figure*}
The failure is structural, not a missing special case. The fact that
