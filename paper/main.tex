...
\item \textbf{Super-analysis} (\S\ref{sec:superanalysis}): an aggressive compilation paradigm
  positioning between conventional heuristic-based optimization and superoptimization in terms of compiler latency and precision. The general recipe for this paradigm is to use solvers (SMT in particular) as an analysis oracle, while applying cheap conventional rules-based transformation for optimization.
...
; (B's two size loads elided)


loop:                          ; vectorized inner loop
%n2  = load i64, ptr %A.size   ; [n2] same address, new load
%idx = add i64 %i, -1          ; zero-based i
|%oob = icmp uge i64 %idx, %n2  ; !(1 <= i <= n2)|
|br i1 %oob, label %trap, label %body|
store <2 x double> %v, ptr %C.data      ; [st]
br label %loop   ; does [st] ever write %A.size?
\end{lstlisting}
\end{minipage}
\caption{The hard case: Julia stdlib GEMM (left: the source, with the
check on \texttt{A[i,l]} spelled out in red; right: the same check
after lowering, laid out line-for-line; \texttt{[n1]}, \texttt{[n2]},
\texttt{[st]} tie the two sides together). Every call to
...
one-based check $1 \le i \le n$ lowers to a single unsigned compare on
the zero-based index, $\mathit{oob} \iff (i{-}1) \ge_u n_2$ (a
negative index wraps to a huge unsigned value). The guard pins
...
the vectorizer has cloned the reload without the scoped-aliasing
metadata that would answer it. Sound encoding must make $n_2$ a fresh
symbol, so every query is trivially satisfiable no matter how many
scalar facts are supplied. What is missing is a \emph{heap invariant}:
...
field tagged n2, the zero-based index computation, in red the unsigned
