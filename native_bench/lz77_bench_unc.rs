// lz77_bench_unc.rs -- UNCHECKED arm of lz77_bench.rs: identical
// workload/output, indexing via get_unchecked. Build with rustc -O
// -C overflow-checks=off. Output must match lz77_bench exactly.
...
fn lz77_scan(data: &[u8], window: usize) -> usize {
    ...
    while i < n {
        ...
        while j < i {
            let mut len = 0usize;
            while i + len < n
                && unsafe { data.get_unchecked(j + len) == data.get_unchecked(i + len) }
            {
                len += 1;
                if len >= 255 { break; }
            }
