...
# Run from repo root:  bash make_context.sh   ->  CONTEXT_ODESSY.md
# =============================================================================
set -u
OUT="CONTEXT_ODESSY.md"
...
desc() {
  case "$1" in
    ...
    run_tests.sh)                 echo "Regression gate. Expected PASS=17/FAIL=6 (heavy/ldeq/stride tests under the light gate BY DESIGN; the two *_sat tripwires must PASS)." ;;
    swift_triage.sh)              echo "Swift static triage: emit-ir -> parse gate -> trap census -> oracle verdicts table (traps/anchored/unsat/sat/vacuous/ldeq)." ;;
    rust_triage.sh)               echo "Rust triage. panic=abort is LOAD-BEARING (unwind emits invoke, invisible to the CallInst hunter). traps=panic." ;;
    julia_triage.sh)              echo "Julia triage via code_llvm module dumps; shape probe prints first bounds sites; traps=bounds_error:boundserror (':' because opt eats commas)." ;;
    run_swift_perf.sh)            echo "The O3-sandwich perf harness: swiftc -O -emit-ir -> opt config -> opt O3 -> llc(-relocation-model=pic, probe-stack strip) -> link. base/base2x round-trip controls, output-equivalence soundness gate, shuffled reps, median-primary. EXTRA_SRCS for multi-file (CryptoSwift)." ;;
    run_zlib_perf.sh)             echo "C compile+runtime benchmark v5: specs none/signed/unsigned/both/anf, base/base2x/oracle, JOBS x THREADS parallel oracle stage, tmpfs corpora, shuffled reps; raw runs in CSV (median via make_perf_report.py)." ;;
    run_zlib.sh)                  echo "Isolated dev harness: zlib/deflate only, analysis+transform tables per (spec x opt), TIER=light|heavy." ;;
    run_zlib_signed.sh)           echo "Older signed-only zlib dev loop (superseded by run_zlib.sh)." ;;
    run_lz4_perf.sh)              echo "lz4 driver-based perf harness (LZ4_compress_HC over tmpfs corpus), TIMEOUT_MS=300 knob." ;;
    run_zstd_audit.sh)            echo "zstd whole-library elimination audit per TU x spec x {light,ldeq}; totals block = the 13.2% number; xxhash vacuous cluster documented in PAPER_FACTS." ;;
    run_matrix.sh)                echo "Sanitizer x benchmark opportunity matrix (C), median-of-N protocol." ;;
    run_matrix_native.sh)         echo "Native-language (Swift/Rust/Julia) opportunity matrix -- the checked-vs-unchecked ceilings." ;;
    run_timeout_sweep.sh)         echo "Per-query timeout sweep (the 52->50@100ms / cliff@1-3ms result)." ;;
    make_perf_report.py)          echo "Median-primary report over perf CSVs (recomputes any statistic from the raw runs_s column)." ;;
    make_matrix_report.py)        echo "Matrix report: median-primary + outlier audit + union columns (all-sanitizers, ANF)." ;;
    plot_smt_latencies.py)        echo "SMT latency distributions by verdict class (SAT/UNSAT/UNKNOWN), log-x histograms + stats CSV." ;;
    plot_matrics.py)              echo "Matrix visualization plots." ;;
    gather_stats.py)              echo "Log aggregation helper." ;;
    strip_traps.py)               echo "Trap-stripping utility for controls." ;;
    patch_zlib_perf_anf.py)       echo "One-shot patcher that added the anf spec to run_zlib_perf.sh (historical)." ;;
    benchmark_commands.sh)        echo "Scratch: assorted benchmark invocations (historical)." ;;
    clone_missing_benchmarks.sh)  echo "Clones benchmark repos beside the ODeSSy checkout." ;;
    test_z3.cpp)                  echo "Z3 linkage smoke test (historical)." ;;
